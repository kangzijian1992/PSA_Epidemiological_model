# ==============================================================================
# 03_incidence_model.R
# 发病率贝叶斯模型构建脚本
# ==============================================================================
#
# Description:
#   使用贝叶斯分层模型估计PsA发病率
#   模型公式: incidence ~ 1 + (1 | Super.region/Region/country) + (1 | study_period)
#           + (1 | sample_size) + (1 | age_group) + (1 | Definition)
#
# Input:
#   - output/All_PsA_Inc-input.Rdata : 预处理后的发病率数据
#
# Output:
#   - output/Inc_model-ALL.Rdata : 拟合的贝叶斯模型
#
# Required R packages:
#   - dplyr
#   - brms
...
cat("=== Loading Preprocessed Incidence Data ===\n")

# 加载预处理数据（如果不存在则运行数据预处理脚本）
if (!file.exists(file.path(output_dir, "All_PsA_Inc-input.Rdata"))) {
...
# 确保person_years为整数
datause$person_years <- as.integer(datause$person_years)
...
# trials(person_years) 表示人年数作为试验次数

Inc_model <- brm(
  formula = Incident.PSA | trials(person_years) ~
    1 +
...
cat("\n=== Model Summary ===\n")
print(summary(Inc_model))

# 保存模型
save(Inc_model, file = file.path(output_dir, "Inc_model-ALL.Rdata"))
cat("\nModel saved to output/Inc_model-ALL.Rdata\n")
...
# 检查R-hat值（应 < 1.01）
cat("\nR-hat values:\n")
print(head(rhat(Inc_model)))

# 检查有效样本量
cat("\nEffective sample sizes:\n")
print(head(neff_ratio(Inc_model)))

cat("\n=== Incidence Model Complete ===\n")